...
Zotero is also a wonderful tool to read and annotate PDFs. Have a look at the [PDF Reader and Note Editor](https://www.zotero.org/support/pdf_reader) guide. It syncs with mobile, so you could also read on your phone or tablet. 
...
- From Zotero, use the command `Citations: Open literature note`. Again, a dropdown opens, with all the references that you added to your exported collection. Chose one, and it will import that reference, alongside all your notes and annotations that you've made in Zotero. You might need to adjust the *Literature note content template* preference in the **Citations** plugin settings.
